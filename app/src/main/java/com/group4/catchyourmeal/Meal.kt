...
//import com.group4.catchyourmeal.adapter.item_combo
//import com.group4.catchyourmeal.adapter.item_meal
...
class Meal : Fragment() {
    ...
    private fun retrievedata() {
        // Toast.makeText(context, "i am in alarcate",Toast.LENGTH_SHORT).show()
        var itemlst:MutableList<Meallist> = mutableListOf<Meallist>()
                ...
                override fun onComplete(@NonNull task: Task<QuerySnapshot>) {
                    if (task.isSuccessful())
                    {
                        for (document : QueryDocumentSnapshot in task.getResult()!!)
                        {
                            ...
                            val price=data.getValue("Meal_price")
                            val mealtype=data.getValue("Meal_type")
                            Log.i("myactivity", "object 3" +data.toString())
                            itemlst.add(count, Meallist(id.toString(),Description.toString(),image.toString(),name.toString(),
                                rating.toString(),price.toString(),mealtype.toString()))

                            // Log.d(TAG, itemlst.get(0).toString())

                            Log.d(TAG, document.getId() + "see data => " + itemlst)
                        }
                        usersRecycler.adapter = item_meal(this@Meal,itemlst)
                    }
                    else
                    {
